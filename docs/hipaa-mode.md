...
Evolute includes a HIPAA compliance mode designed for use in healthcare environments where Protected Health Information (PHI) may appear on screen.
...
HIPAA mode in Evolute is one piece of the puzzle. Full compliance requires:
...
- Log who accessed Evolute, when, and what type of queries were made
...
For organizations that cannot send any data externally, Evolute supports pointing at a custom API endpoint:
...
Evolute is a tool - enabling HIPAA mode does not make your organization HIPAA compliant on its own. Compliance requires organizational policies, technical safeguards, staff training, and BAAs with all service providers. Consult your compliance officer before deploying in a healthcare setting.
