...
Evolute supports voice input (speech-to-text) and spoken responses (text-to-speech). Both are optional - you can use Evolute with just text input and silent responses.
...
3. In Evolute, open Settings (tray > Settings)
...
> Evolute currently hard-codes `ggml-base.bin` as the model filename - if you want to use a different model, either rename your file or update the path in `src/services/transcription/whisper-local.ts`.
...
3. Release - Evolute transcribes and sends your question with a screenshot

## Text-to-Speech (TTS)

TTS makes Evolute speak its responses aloud.
...
3. Copy your API key
4. In Evolute Settings, paste it in the **ElevenLabs API Key** field
