...
By default, Evolute calls APIs directly using your keys. For team deployments, you can route all API calls through a proxy server - useful for centralized key management, usage tracking, and access control.
...
After deploy, Wrangler outputs your worker URL (e.g., `https://evolute-proxy.your-subdomain.workers.dev`).
...
Then in Evolute, the app sends the token with each request.
