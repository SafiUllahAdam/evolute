...
Evolute Windows uses a **BYOK (Bring Your Own Keys)** model. All API keys are stored locally on your machine and sent directly to providers - there is no intermediary proxy server by default.

```
Your Machine                          Cloud APIs
┌─────────────────┐                  ┌──────────────┐
│ Evolute App      │ ──── HTTPS ───→  │ Anthropic    │
│                 │ ──── HTTPS ───→  │ OpenAI       │
...
- API keys are stored in `%APPDATA%/evolute-windows/settings.json`
...
| Settings/keys | `%APPDATA%/evolute-windows/settings.json` | Yes |
