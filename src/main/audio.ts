...
import { transcribeAssemblyAI } from "../services/transcription/assemblyai-batch";
 ...
 *      - "assemblyai"    → upload the WAV to AssemblyAI and poll for the result
 *      - "openai"        → wrap PCM in a WAV header and POST to Whisper
...
export class AudioCapture {
  ...
  private async transcribe(pcmBuffer: Buffer): Promise<string> {
    ...
    const assemblyKey = this.settings.get("assemblyaiApiKey");
    ...
    // AssemblyAI's own REST API - the streaming provider does not fit this
    // pipeline, which hands over one finished recording rather than a stream.
    if (provider === "assemblyai" && assemblyKey) {
      return transcribeAssemblyAI(pcmBuffer, assemblyKey);
    }
    ...
    // Fallbacks: use whichever key is present, regardless of the setting.
    if (assemblyKey) {
      return transcribeAssemblyAI(pcmBuffer, assemblyKey);
    }
    if (openaiKey) {
    ...
    throw new Error(
      "No transcription provider configured. Set transcriptionProvider to 'whisper-local', or add an AssemblyAI or OpenAI API key."
    );
  }
  ...
  private async transcribeWhisper(
    ...
    const boundary = "----EvoluteAudio" + Date.now();
