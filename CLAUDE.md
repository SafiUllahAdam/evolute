...
Evolute Windows is an Electron + TypeScript port of
[farzaa/clicky](https://github.com/farzaa/clicky). It's a tray-resident AI
screen companion: the user speaks or types, Evolute captures a screenshot,
...
- A transparent window that renders nothing looks identical to a window
  that failed to open. Check the terminal for `[Evolute] Overlay window
...
- API keys (Anthropic, OpenAI, ElevenLabs, AssemblyAI) live in
  `%APPDATA%/evolute-windows/settings.json` in plain text. Acceptable for a
